...
const AdminDashboard = () => {
  ...
  const validation = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      adharNumber: Yup.string()
        .matches(/^\d{12}$/, "Adhar Number must be between 12 digits.")
    ...
    onSubmit: (value) => {
      if (isEdit) {
        ...
        dispatch(updateSubAdminUser(updateUSer));
        ...
        dispatch(addNewSubAdminUser(newUser));
        validation.resetForm();
      }
  ...
  const handleDeleteUser = () => {
    if (user && user._id) {
      dispatch(deleteSubAdminUser(user._id));
    }
    setUser("");
  ...
  const handleActiveInactiveUser = () => {
    ...
    if (user && user._id) {
      dispatch(activeInactiveSubAdminUser({ user: user._id, status }));
    }
    setUser("");
  ...
  useEffect(() => {
    if (role && (role.role === "sub_admin" || role.role === "user")) {
  ...
  const columns = useMemo(
    () => [
      ...
      { header: "Adhar Number", accessorKey: "adharNumber" },
      {
        header: "Status",
      ...
      <Container fluid className="py-4 ">
        <Row className="mb-4">
          <Col>
            ...
            <span className="text-muted">Manage Sub Admin</span>
          </Col>
        </Row>
        {loading ? (
                    ...
                    <>
                      ...
                      <div className="mb-3">
                        ...
                        <Input
                          ...
                          onChange={validation.handleChange}
                          onBlur={validation.handleBlur}
                          value={validation.values.adharNumber || ""}
                          invalid={
                            validation.touched.adharNumber &&
                            validation.errors.adharNumber
                              ? true
                              : false
                          }
                        />
